...
for i in range(len(users)):
    ...
    with pm.Model() as lognormal_PG:
        lam_tau = pm.Exponential('lam_tau',lam=1,shape=1)
        eta_tau = pm.Exponential('eta',lam=1,shape=1)
        K_tau = lam_tau**2 * pm.gp.cov.ExpQuad(1,eta_tau)
        mean = pm.gp.mean.
        gp = pm.gp.Marginal(mean_func=mean, cov_func=K_tau)
